...
def _call_text_moderation(
    text: str, span: trace.Span
    ...
    if not response.ok:
        raise RuntimeError(
            f"Moderation service unavailable. Please try again later. {response.text}"
        )
...
def _call_media_moderation(
    media: str, span: trace.Span
) -> Tuple[dict[str, Any], str, str, str]:
    """
...
def check_content_safety(
    *, text: str | None = None, media: str | None = None
    ...
    with tracer.start_as_current_span("moderate_text") as span:
        ...
        if text is not None:
            result, feedback, content_type, mime_type = _call_text_moderation(
                text, span
            )

        elif media is not None:
            result, feedback, content_type, mime_type = _call_media_moderation(
                media, span
            )
...
class ChatSessionWithTracing:
    ...
    async def chat_with_gemini(
        self, message: dict, history: List, past_messages: List
    ) -> Tuple[str, List, str]:
        """
        ...
        with tracer.start_as_current_span(
            ...
            logger.info(
                f"New turn - Text: '{message.get('text', '')[:50]}...', Files: {len(message.get('files', []))}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Response generated ({len(result.all_messages())} messages in history)"
                )
...
def create_chat_interface() -> gr.Blocks:
    ...
    with gr.Blocks(
        title="ACME Customer Service Training Agent", fill_height=True
    ) as demo:
            ...
            with gr.Column(scale=3):
                ...
                gr.ChatInterface(
                    ...
                    chatbot=gr.Chatbot(
                        ...
                        type="messages",
                        placeholder="👋 Start by greeting the customer or introducing yourself.",
                        height="75vh",
                    ),
                    # TODO: pass state and feedback as additional I/O
